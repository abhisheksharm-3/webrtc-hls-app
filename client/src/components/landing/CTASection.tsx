...
/**
 * @description A single statistic item for display in the CTA section.
 */
const StatItem = ({ value, label }: { value: string; label: string }) => (
  <div className="px-4 text-center sm:px-8">
    <div className="font-mono text-3xl font-bold tracking-tighter sm:text-4xl">{value}</div>
    <div className="text-sm text-muted-foreground">{label}</div>
  </div>
);

/**
 * @description A call-to-action section for the landing page, encouraging users to start streaming.
 */
export function CTASection() {
  // ✅ Simplified to use the hook directly without renaming.
          ...
          <div className="relative mx-auto max-w-3xl space-y-8">
            <div className="space-y-4">
              ...
              <p className="mx-auto max-w-2xl text-lg text-muted-foreground sm:text-xl">
                Join thousands of creators using our platform for professional live broadcasting and real-time communication.
              </p>
            </div>
            ...
            {/* ✅ Using declarative `divide-x` for separators. */}
