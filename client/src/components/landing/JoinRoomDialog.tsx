...
const radioLabelStyles =
  "flex items-center space-x-2 p-3 rounded-lg border border-border/50 hover:border-border/80 transition-colors has-[:checked]:border-primary/50 has-[:checked]:bg-primary/10 cursor-pointer";

/**
 * A type guard to check if a string is a valid JoinModeType.
 * @param {string} value - The value to check.
 * @returns {boolean} - True if the value is a valid join mode.
 */
const isJoinMode = (value: string): value is JoinModeType => {
  return value === "participate" || value === "watch";
};
 ...
 */
const JoinModeSelector = ({
  value,
  onValueChange,
}: {
  value: JoinModeType;
  onValueChange: (value: JoinModeType) => void;
}) => (
  <div className="grid gap-3">
    ...
    >
      <Label htmlFor="participate" className={radioLabelStyles}>
        <RadioGroupItem value="participate" id="participate" />
        <div className="flex-1">
          <div className="flex items-center gap-2 font-semibold">
            <Users className="h-4 w-4 text-primary" /> Join as Guest
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            Participate with video and audio.
          </p>
        </div>
      </Label>
      <Label htmlFor="watch" className={radioLabelStyles}>
        <RadioGroupItem value="watch" id="watch" />
        <div className="flex-1">
          <div className="flex items-center gap-2 font-semibold">
            <Eye className="h-4 w-4 text-primary" /> Watch Only
          </div>
          <p className="text-xs text-muted-foreground mt-1">
            Watch the HLS stream without joining.
          </p>
        </div>
      </Label>
    </RadioGroup>
