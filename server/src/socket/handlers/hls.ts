...
export function registerHlsHandlers(io: Server, socket: Socket): void {
  ...
  socket.on(
    "start-hls",
    async (data: { roomId: string } /* : StartHlsPayload */) => {
      try {
        const { roomId } = data;
        ...
        if (!participant || !participant.isHost) {
          return socket.emit("error", {
            message: "Only the host can start the HLS stream.",
          });
        }
        if (!room) {
          return socket.emit("error", {
        ...
        if (room.hlsProcess || room.appData.hls?.isStartingHls) {
          logger.warn(
            `Attempted to start HLS for room ${roomId}, but it is already running or starting.`
          );
          return socket.emit("error", {
            message: "HLS stream is already running or starting.",
          });
        }

        // Acquire the lock
        room.appData.hls = { isStartingHls: true };

        // --- Start the HLS Stream ---
        logger.info(
          `Host ${participant.id} is starting HLS for room ${roomId}`
        );
        ...
        logger.info(
          `✅ HLS stream started for room ${roomId} at ${playlistUrl}`
        );
      } catch (error) {
        logger.error("Error starting HLS stream:", error);
        const room = getLiveRoom(data.roomId);
        if (room) {
          room.appData.hls = { isStartingHls: false };
        }
  ...
  socket.on("stop-hls", async (data: { roomId: string }) => {
    try {
      ...
      if (!participant || !participant.isHost) {
        return socket.emit("error", {
          message: "Only the host can stop the HLS stream.",
        });
      }
      if (!room) {
        return socket.emit("error", {
